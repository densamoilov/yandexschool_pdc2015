...
* kmeans.cpp: k-means OpenMP version
*/
...
vector<size_t> KMeans(const Points& data, size_t K) {
    ...
    while (!converged) {    	
        ...
        #pragma omp parallel for schedule(dynamic, data_size / nthreads) 
        ...
        #pragma omp parallel
        {
            Points part_centroids(K);
            std::vector<size_t> part_clusters_sizes(K);
            part_centroids.assign(K, Point(dimensions));
            #pragma omp for
            for (size_t i = 0; i < data_size; i++) {
                for (size_t d = 0; d < dimensions; d++) {				
                    part_centroids[clusters[i]][d] += data[i][d];
                }
                part_clusters_sizes[clusters[i]]++;
            }
            for (size_t i = 0; i < K; i++) {
                for (size_t d = 0; d < dimensions; d++) {
                    #pragma omp atomic
                    centroids[i][d] += part_centroids[i][d];
                }
                #pragma omp atomic
                clusters_sizes[i] += part_clusters_sizes[i];
            }
        }
